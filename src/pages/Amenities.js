...
const { Option } = Select;
const Amenities = () => {
    const { canDelete } = usePermission("/amenities");
    ...
    const { loading: getOrderUserLoading, Amenitiesres: getOrderResponse } =
        useSelector((state) => state.Amenitiesres);

    const { loading: EditcategoryLoading, EditAmenitiesres: EditcategoryResponse } =
        useSelector((state) => state.EditAmenitiesres);
        ...
        },
        {
            ...
            render: (_, record) => {
                ...
                const imageUrl = isFullURL
                    ? record.image
                    : `${IMAGE_BASE_URL}amenities/${record.image}`;
    ...
    const showDeleteConfirm = (record) => {
        confirm({
            ...
            onOk() {
                console.log("OK");
                dispatch(DeleteAmenitiesById(record.am_id))
                    .then(() => dispatch(GetAmenitiesById()))
    ...
    const handleSaveEdit = (editedData) => {
        ...
        formData.append("name", editedData.name);
        formData.append('folder', 'amenities'); // or 'specifications', 'gallery', 'slider'
        ...
        if (editData.am_id) {
            dispatch(EditAmenitiesById(editData.am_id, formData));
        } else {
            dispatch(AddAmenitiesById(formData));
        }
