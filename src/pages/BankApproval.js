...
import {
    GetBankApprovalById,
    AddBankApprovalById,
    EditBankApprovalById,
    DeleteBankApprovalById,
} from "../reducer/thunks";
...
const { Option } = Select;
const BankApproval = () => {
    const { canDelete } = usePermission("/bank-approval");
    const [file, setFile] = useState(null);
    const fileInputRef = useRef(null);
    const { loading: getcategoriesLoading, BankApprovalres: getcategoriesResponse } =
        useSelector((state) => state.BankApprovalres);
    ...
    const { loading: getOrderUserLoading, BankApprovalres: getOrderResponse } =
        useSelector((state) => state.BankApprovalres);

    const { loading: EditcategoryLoading, EditBankApprovalres: EditcategoryResponse } =
        useSelector((state) => state.EditBankApprovalres);
        ...
        },
        {
            ...
            render: (_, record) => {
                ...
                const imageUrl = isFullURL
                    ? record.image
                    : `${IMAGE_BASE_URL}bank_app/${record.image}`;
    ...
    const showDeleteConfirm = (record) => {
        confirm({
            ...
            onOk() {
                console.log("OK");
                dispatch(DeleteBankApprovalById(record.bank_id))
                    .then(() => dispatch(GetBankApprovalById()))
    ...
    const handleSaveEdit = (editedData) => {
        ...
        formData.append('folder', 'bank_app'); // or 'specifications', 'gallery', 'slider'
        ...
        if (editData.bank_id) {
            dispatch(EditBankApprovalById(editData.bank_id, formData));
        } else {
            dispatch(AddBankApprovalById(formData));
        }
