...
const { Option } = Select;
const PropStatus = () => {
    const { canDelete } = usePermission("/property-status");
    const [file, setFile] = useState(null);
    const fileInputRef = useRef(null);
    const { loading: getcategoriesLoading, PropStatusres: getcategoriesResponse } =
        useSelector((state) => state.PropStatusres);
    ...
    const { loading: getOrderUserLoading, PropStatusres: getOrderResponse } =
        useSelector((state) => state.PropStatusres);

    const { loading: EditcategoryLoading, EditPropStatusres: EditcategoryResponse } =
        useSelector((state) => state.EditPropStatusres);
    ...
    const showDeleteConfirm = (record) => {
        confirm({
            title: `Are you sure delete this ${record.status}?`,
            icon: <DeleteFilled />,
            ...
            onOk() {
                console.log("OK");
                dispatch(DeletePropStatusById(record.status_id))
                    .then(() => dispatch(GetPropStatusById()))
    ...
    const handleSaveEdit = (editedData) => {
        ...
        if (editData.status_id) {
            dispatch(EditPropStatusById(editData.status_id, editedData));
        } else {
            dispatch(AddPropStatusById(editedData));
        }
