...
import {
    ...
    AddLegalApprovalById,
    EditLegalApprovalById,
    DeleteLegalApprovalById,
} from "../reducer/thunks";
...
const LegalApproval = () => {
    ...
    const { loading: getcategoriesLoading, LegalApprovalres: getcategoriesResponse } =
        useSelector((state) => state.LegalApprovalres);
    ...
    const { loading: getOrderUserLoading, LegalApprovalres: getOrderResponse } =
        useSelector((state) => state.LegalApprovalres);

    const { loading: EditcategoryLoading, EditLegalApprovalres: EditcategoryResponse } =
        useSelector((state) => state.EditLegalApprovalres);
        ...
        },
        {
            ...
            render: (_, record) => {
                ...
                const imageUrl = isFullURL
                    ? record.image
                    : `${IMAGE_BASE_URL}legal_app/${record.image}`;
    ...
    const showDeleteConfirm = (record) => {
        confirm({
            ...
            onOk() {
                console.log("OK");
                dispatch(DeleteLegalApprovalById(record.legal_id))
                    .then(() => dispatch(GetLegalApprovalById()))
    ...
    const handleSaveEdit = (editedData) => {
        ...
        formData.append('folder', 'legal_app'); // or 'specifications', 'gallery', 'slider'

        if (editedData.image && editedData.image[0]) {
            formData.append("image", editedData.image[0]);
        }

        if (editData.legal_id) {
            dispatch(EditLegalApprovalById(editData.legal_id, formData));
        } else {
            dispatch(AddLegalApprovalById(formData));
        }
