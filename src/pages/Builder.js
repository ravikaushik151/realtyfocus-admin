...
const { Option } = Select;
const Builder = () => {
  const { canDelete } = usePermission("/builder");
  const [file, setFile] = useState(null);
  const fileInputRef = useRef(null);
  const { loading: getcategoriesLoading, Builderres: getcategoriesResponse } =
    useSelector((state) => state.Builderres);
  ...
  const { loading: getOrderUserLoading, Builderres: getOrderResponse } =
    useSelector((state) => state.Builderres);

  const { loading: EditcategoryLoading, EditBuilderres: EditcategoryResponse } =
    useSelector((state) => state.EditBuilderres);
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "logo",
      render: (_, record) => {
        const isFullURL = record.logo?.startsWith("https://storage.googleapis.com/");
        const imageUrl = isFullURL
          ? record.logo
          : `${IMAGE_BASE_URL}builder/${record.logo}`;
  ...
  const showDeleteConfirm = (record) => {
    confirm({
      ...
      onOk() {
        console.log("OK");
        dispatch(DeleteBuilderById(record.builder_id))
          .then(() => dispatch(GetBuilderById()))
  ...
  const handleSaveEdit = (editedData) => {
    ...
    formData.append('folder', 'builder'); // or 'specifications', 'gallery', 'slider'

    // if (file) {
    //     formData.append("imageFile", file);
    // }

    if (editedData.logo && editedData.logo[0]) {
      formData.append("logo", editedData.logo[0]);
    }

    if (editData.builder_id) {
      dispatch(EditBuilderById(editData.builder_id, formData));
    } else {
      dispatch(AddBuilderById(formData));
    }
