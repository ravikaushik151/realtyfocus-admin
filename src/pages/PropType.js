...
const { Option } = Select;
const PropType = () => {
    const { canDelete } = usePermission("/property-type");
    const [file, setFile] = useState(null);
    const fileInputRef = useRef(null);
    const { loading: getcategoriesLoading, PropTyperes: getcategoriesResponse } =
        useSelector((state) => state.PropTyperes);
    ...
    const { loading: getOrderUserLoading, PropTyperes: getOrderResponse } =
        useSelector((state) => state.PropTyperes);

    const { loading: EditcategoryLoading, EditPropTyperes: EditcategoryResponse } =
        useSelector((state) => state.EditPropTyperes);
    ...
    const showDeleteConfirm = (record) => {
        confirm({
            ...
            onOk() {
                console.log("OK");
                dispatch(DeletePropTypeById(record.type_id))
                    .then(() => dispatch(GetPropTypeById()))
    ...
    const handleSaveEdit = (editedData) => {
        ...
        // }
        if (editData.type_id) {
            dispatch(EditPropTypeById(editData.type_id, editedData));
        } else {
            dispatch(AddPropTypeById(editedData));
        }
